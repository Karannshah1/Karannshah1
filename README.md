<div align="center">

# 👋 Hi, I'm <b>Karan Shah</b>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=300&color=gradient&customColorList=12,20,24&text=Software%20Engineer&fontColor=ffffff&fontSize=58&fontAlignY=48&animation=fadeIn"/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&duration=3200&pause=1200&color=8B5CF6&center=true&vCenter=true&width=950&lines=Java+%7C+Virtual+Threads;Distributed+Systems+%7C+Microservices;Kafka+%7C+gRPC+%7C+PostGIS;Clean+Architecture+%7C+SOLID;Open+to+Software+Development+Engineer+Roles)](https://git.io/typing-svg)

<br>

<a href="mailto:info.karanjshah@gmail.com">
<img src="https://skillicons.dev/icons?i=gmail" height="48"/>
</a>
&nbsp;&nbsp;&nbsp;

<a href="https://www.linkedin.com/in/karanshaah/">
<img src="https://skillicons.dev/icons?i=linkedin" height="48"/>
</a>
&nbsp;&nbsp;&nbsp;

<a href="https://leetcode.com/u/karannshah/">
<img src="https://cdn.simpleicons.org/leetcode/FFA116" height="48"/>
</a>

<br><br>

<img src="https://komarev.com/ghpvc/?username=Karannshah1&style=for-the-badge&label=PROFILE+VIEWS&color=8B5CF6"/>

<img src="https://img.shields.io/github/followers/Karanshah1?style=for-the-badge&logo=github&label=FOLLOWERS&color=8B5CF6"/>

</div>

---

# 💼 About Me

Systems Engineer focused on backend system architecture, high-scalability distributed systems, microservices, and performance optimization.

- 🎓 B.Tech Computer Science and Engineering, Nirma University (2024)
- ☕ Java 21 as primary language (Virtual Threads, Concurrency)
- 🌱 Deep-diving into Low-Latency Transactions, Kafka, and gRPC
- 🏆 Competitive Programming: LeetCode (Rating: 1692) | Codeforces (Rating: 1438)
- 📫 info.karanjshah@gmail.com

---
# ⚙️ Tech Stack

## ☕ Languages

![Languages](https://skillicons.dev/icons?i=java,cpp,c,python)

---

## 🚀 Frameworks & Technologies

![Frameworks](https://skillicons.dev/icons?i=spring,kafka,redis)

<p>
<img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"/>
<img src="https://img.shields.io/badge/Apache_Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white"/>
<img src="https://img.shields.io/badge/gRPC-244C5A?style=for-the-badge&logo=grpc&logoColor=white"/>
<img src="https://img.shields.io/badge/Microservices-02569B?style=for-the-badge"/>
</p>

---

## 🗄️ Databases

![Databases](https://skillicons.dev/icons?i=postgres,mysql)

<p>
<img src="https://img.shields.io/badge/PostGIS-336791?style=for-the-badge&logo=postgresql&logoColor=white"/>
</p>

---

## 🏛️ Software Architecture

<p>
<img src="https://img.shields.io/badge/Clean_Architecture-24292F?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Distributed_Systems-24292F?style=for-the-badge"/>
<img src="https://img.shields.io/badge/SOLID_Principles-24292F?style=for-the-badge"/>
<img src="https://img.shields.io/badge/OOP-24292F?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Design_Patterns-24292F?style=for-the-badge"/>
</p>

---

## 🧠 Computer Science Fundamentals

<p>
<img src="https://img.shields.io/badge/Data_Structures_&_Algorithms-00599C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/DBMS-00599C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Operating_Systems-00599C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Computer_Networks-00599C?style=for-the-badge"/>
</p>

---

## 🛠️ Development Tools

![Tools](https://skillicons.dev/icons?i=git,github,maven,idea,vscode,postman,docker)

---

# 🚀 Featured Projects

<details>
<summary><b>🗺️ GeoFleet</b></summary>

- High-frequency location tracking system.
- Designed utilizing **gRPC** bidirectional streaming and **Apache Kafka**.
- Database management with **PostGIS** for geospatial querying.
- Engineered for real-time driver-rider matching and low-latency transactions.

</details>

---

# 💼 Experience

### Systems Engineer

**Tata Consultancy Services (TCS) — Gandhinagar**

- Engineering scalable backend services and APIs.
- Focusing on performance optimization and reliable database architecture.
- Adhering to clean code principles (SOLID) and design patterns for robust application delivery.

---

# 📈 GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-streak-stats-eight.vercel.app/?user=Karannshah1&theme=tokyonight&hide_border=true"/>


---

# 📊 Contribution Graph

![](https://github-readme-activity-graph.vercel.app/graph?username=Karannshah1&theme=tokyo-night)

---
# 🐍 Contribution Snake

<p align="center">
  <img src="https://raw.githubusercontent.com/Karannshah1/Karannshah1/output/github-contribution-grid-snake-dark.svg" alt="Contribution Snake"/>
</p>

---

# 🎯 Current Focus

```yaml
Learning:
  - Advanced Java 21 Features
  - Low-Latency Systems
  - System Design
  - Cloud Architecture

Building:
  - GeoFleet
  - High-Frequency Distributed Applications

Competitive Programming:
  - LeetCode (Algorithms & Data Structures)
  - Codeforces

OpenTo:
  - Software Development Engineer Roles
